# Spec: File-Based Sync Configuration

**Change ID:** `sync-config-file`

**Capability:** `file-config`
...
- **Related Files:**
  - `src/sync/config-loader.ts`: Modified for file reading
  - `src/scheduler.ts`: Updated to use file-based loader
  - `config/sync-rules.json`: New example config file
- **Related Environment Variables:** `SYNC_CONFIG_PATH`
